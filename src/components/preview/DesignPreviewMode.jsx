...
function DesignPreviewMode({ data, renderTopNav, userId: propUserId }) {
    ...
    if (Array.isArray(initialReviews) && initialReviews.length > 0) {
      return initialReviews
        .map((r) => ({
          ...
          // FIXED: Use CLIENT information (the person who gave the review)
          author: r.client?.attributes
            ? `${r.client.attributes.firstName || ""} ${
                r.client.attributes.lastName || ""
              }`.trim()
            : r.author || r.name || "Anonymous",

          // FIXED: Use CLIENT avatar (the person who gave the review)
          avatar: r.client?.attributes?.avatar || null,
  ...
  useEffect(() => {
    const loadReviews = async () => {
      ...
      if (userId) {
        ...
        // FIXED: Map to expected fields for PreviewReviews - use CLIENT info
  ...
  const getFontField = () => {
    const localFont = effectiveData?.profileDesign?.layout?.font;
    if (localFont && localFont !== "undefined" && localFont !== "") return localFont;
    const profileFont = effectiveProfile?.profileDesign?.layout?.font;
    if (profileFont && profileFont !== "undefined" && profileFont !== "") return profileFont;
    if (effectiveData?.layout?.font && effectiveData.layout.font !== "undefined" && effectiveData.layout.font !== "") return effectiveData.layout.font;
    if (data?.layout?.font && data.layout.font !== "undefined" && data.layout.font !== "") return data.layout.font;
    return "inter";
  };
  const fontValue = getFontField();
  const fontFamily = FONT_MAP[fontValue] || FONT_MAP["inter"];
  // Log para depuración
  console.log("[Profile Preview] fontValue:", fontValue, "fontFamily:", fontFamily);
          ...
          {showSubmitButton && (
            <Button
              ...
              sx={{
                ...
                "&:hover": {
                  backgroundColor: themeColors.main + "CC",
                },
              }}
              onClick={() => {
                const userId = effectiveProfile?.id || effectiveProfile?.userId;
                if (userId) {
...
function PreviewGeneral({
  ...
  const renderSection = (title, children) => (
    ...
    >
      <Typography variant="h2" sx={{ mb: 2, color: themeColors.main }}>
        {title}
      </Typography>
...
function PreviewWorkflow({
  ...
  // Match ViewUserProfile section style
  const renderSection = (title, children) => (
    <Paper
      elevation={0}
      sx={{
